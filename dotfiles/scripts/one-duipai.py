...
example_text = '''example:
  oi-show.py
  oi-show.py -o sol -r std -i test.in
  oi-show.py -o sol -r std -i test.in -w 80'''

parser = argparse.ArgumentParser(
    description="OI程序对拍工具 - 展示输入文件内容并对比两个程序的运行结果",
    epilog=example_text,
...
os.system("diff -W {width} -y 1.out 2.out ".format(**args))
