  ...
  // console.log(RbookEntity.AllMarkdownFiles)
    ...
    function deal_code_template(md: Markdown, d: CodeTemplate, code_template: CodeTemplate[]) {
        ...
        if( Path.isAbsolute(code_path) )
          code_path = Path.join(__code_template_dir, code_path.replace(/^\/code\/?/, ''))
        else
          code_path = Path.join(Path.dirname(file_dir),d.code)
        
        // 相对__workdir的路径
        let code_path_relative = code_path.startsWith(__code_template_dir)
          ? Path.join('code', Path.relative(__code_template_dir,code_path))
          : Path.relative(__workdir,code_path)
