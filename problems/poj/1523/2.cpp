...
const int maxn = 1005;  //
const int maxe = 4005;  // 4e6+5 改为常量
const int mod = 1000000007;  // 1e9+7 改为常量
    ...
    while (1) {
        ...
        cout << "Network #" << ++case_id << "\n";
         ...
         for (int i = 1; i <= max_n; i++) {
             // cout << i << " : " << tjcut.subnet_cnt[i] << "\n";
             if (tjcut.subnet_cnt[i] >= 2) { // 至少分成两块才叫 SPF
                 has_spf = true;
                 cout << "  SPF node " << i << " leaves " << tjcut.subnet_cnt[i] << " subnets\n";
             }
         }
         if (!has_spf) cout << "  No SPF nodes\n";
         cout << "\n";
