...
bool find_del_edge(int u, int fa, int target) {
    ...
    for(int i = 0; i < adj[u].size(); i++) {
        ...
        // 计算如果不走 v，回溯后的最佳选择 (next_candidate)
        // 逻辑：在 adj[u] 中找排在 v 后面且不是 fa 的第一个邻居
        int next_candidate = target;
        for(int j = i + 1; j < adj[u].size(); j++) {
            if (adj[u][j].first != fa) {
                next_candidate = adj[u][j].first;
                break;
            }
        }

        // 只要是环边，且 v 比 回头路(next_candidate) 还要差，就断开
        if(on_ring_edge[eid] && v > next_candidate) {
            del_edge = eid;
            return 1;
        }

        if(find_del_edge(v, u, next_candidate)) return 1;
    }
    return 0;
}

// 找倒要删除的边
bool find_del_edge_old(int u,int fa,int target) {
