...
class Lb {
  constructor(options) {
    ...
    options = Object.assign({
	    ...
	    moniterKeyEvent: false,  // 是否监听键盘事件
	    moniterTouchEvent: true  // 是否监听屏幕滑动事件
    }, options)

    this.lbBox = document.getElementById(options.id);
    this.lbContent = document.createElement("div")
    this.lbContent.setAttribute("class", "lb-content")
    //get all images
    let images = this.lbBox.querySelectorAll('img')
    
    images.forEach( (img,i) => { img.setAttribute("class", i==0 ?"lb-item active" : "lb-item"); this.lbContent.appendChild(img)})
    this.lbBox.append(this.lbContent)
    this.lbItems = this.lbBox.querySelectorAll('.lb-item');

    this.lbOl= document.createElement("ol")
    this.lbOl.setAttribute("class", "lb-sign")
    this.lbOl.innerHTML= Array(this.lbItems.length).fill(0).map( (e,i)=> {return i==0 ?`<li class="active">${i+1}</li>` : `<li>${i+1}</li>`}).join("\n");
    this.lbBox.appendChild(this.lbOl)
    this.lbSigns = this.lbBox.querySelectorAll('.lb-sign li');

    this.lbCtrlL = document.createElement("div")
    this.lbCtrlL.setAttribute("class", "lb-ctrl left")
    this.lbCtrlL.innerText = "＜"
    this.lbCtrlR = document.createElement("div")
    this.lbCtrlR.setAttribute("class", "lb-ctrl right")
    this.lbCtrlR.innerText = "＞"
    this.lbBox.append(this.lbCtrlL,this.lbCtrlR)

    //删除 p 元素
    this.lbBox.querySelectorAll("p").forEach( e => this.lbBox.removeChild(e))
