...
开发环境中，`/api`、`/api/*` 的成功和错误响应统一包含：

```http
Cache-Control: no-store
```

这样开发模式下修改文章并重新请求时不会读到浏览器缓存。生产环境唯一的例外是 `GET /api/catalog`：它返回 `Cache-Control: public, max-age=60`，方便题目解析站在服务端短暂缓存文章目录；其他 API 仍使用 `no-store`。目录响应中的内容仍以服务端重建后的索引为准。
