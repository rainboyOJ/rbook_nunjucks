...
- [ ] 抽取 VPS 上的通用部署逻辑，例如 `scripts/deploy-vps.sh`。
- [ ] 支持“新镜像部署”和“只更新文章内容部署”两个模式。
- [ ] 保留当前镜像构建完成后再部署的依赖关系。
- [ ] 保留文章-only 修改时不重新构建 Docker 的策略。
- [ ] 统一 sparse checkout、容器测试构建、容器启动、健康检查逻辑。
- [ ] 更新 GitHub Actions，只负责准备参数和调用脚本。

### 验收标准

- 两个 workflow 中不再维护两份大段重复 shell。
- 修改部署逻辑时只需要改一个脚本。
- Docker 镜像部署和内容部署仍然都能工作。
