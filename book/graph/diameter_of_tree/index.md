...
1. $x_1 + x_3 > x_1 + x_2$
2. 于是得到 $L_{au} +x1 + x3 >L_{au} +x_1 + x_2$
3. 于是得到了一条比直径$L_{ab}$更长的路径, 矛盾.
...
1. 因为树上的任意两个点都存在唯一的路径, $u$可以到达直径$(a,b)$上的任意一点, 设$u$到$a$的距离为$L_{ut}$,那么$t$点就是最近的点
2. 显然可以得到 $L_{ut} + L_{tb} < L_{ut} + L_{uv}$(因为$v$是$u$的最远点)
3. 于是得到 $L_{uv} > L_{ub}$
4. 但是根据$v$的定义, $L_{av} > L_{ab}$
5. 矛盾
