...
**算法步骤**:
0. 先排序
1.  初始化左指针 `left = 0`，右指针 `right = n - 1`（`n` 为数组长度）。
2.  当 `left < right` 时，进行循环：
    a. 计算当前指针指向的两个元素的和 `current_sum = nums[left] + nums[right]`。
    b. 如果 `current_sum == target`，则找到了目标对，返回 `left` 和 `right`。
    c. 如果 `current_sum < target`，说明和太小了。由于数组是排序的，需要增大和，因此将左指针向右移动 `left++`。
    d. 如果 `current_sum > target`，说明和太大了。需要减小和，因此将右指针向左移动 `right--`。
3.  如果循环结束仍未找到，则说明不存在这样的整数对。
...
1. 答案必然在区间$[i,j]$内
2. 如果$a[i] + a[j] < target$,那么$a[i] + a[k] < target  \quad  i< k < j$,这说明$a[i]$太小了,**永远不可能是答案**,那么可以把$i$从答案区间里面删除,即$i++$,这样新的答案区间就变成了$[i+1,j]$
