...
### 逐行模拟：`text = "abababa"` 搜索 `pattern = "aba"`
...
用「逐行模拟：用 `"ababcabab"` 跑一遍构造过程」一节的 `"ababcabab"` 画失配树：节点 `k` 代表**长度为 `k` 的前缀**，它的父节点是 `pi[k-1]`。
