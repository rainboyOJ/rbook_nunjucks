...
real[] points = {0,4,5,2.5};
string[] names = {"$i$","$j-1$","$j$","$p$"};
...
draw_square_brace(h1,points[0],points[1],defaultpen);
...
path b1 = shift((0,-0.3)) * brace(j,p1) ;
draw(b1);
label("can",b1,S);
...
dot(p1_1);
label("$p-1$",p1_1,N);
path b2 = shift((0,-0.3)) * brace(p1_1,i) ;
draw(b2);
label("can not",b2,S);